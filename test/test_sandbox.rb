...
class TestSandbox < Minitest::Test
  def setup
    ...
    FileUtils.mkdir_p(File.join(@project_path, ".aura", "bin"))
    FileUtils.mkdir_p(File.join(@project_path, ".aura", "tools", "hello"))
    ...
    File.write(File.join(@project_path, ".aura", "bin", "sandbox-wrapper"), <<~SH)
      #!/bin/bash
      echo "===SANDBOXED==="
      exec "$@"
    SH
    FileUtils.chmod(0755, File.join(@project_path, ".aura", "bin", "sandbox-wrapper"))

    # Create a simple Python tool
    File.write(File.join(@project_path, ".aura", "tools", "hello", "logic.py"), <<~PY)
      import sys, json
      print(json.dumps({"status": "ok", "message": "hello from tool"}))
    PY
    File.write(File.join(@project_path, ".aura", "tools", "hello", "manifest.json"), <<~JSON)
      {
        "name": "hello",
