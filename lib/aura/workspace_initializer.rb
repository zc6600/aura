...
module Aura
  ...
  module WorkspaceInitializer
    ...
    def self.handle_no_workspace(_start_dir)
      puts "\e[33m⚠️ Warning: Not in an Aura workspace (no .aura folder found in parent directories).\e[0m"

      use_sandbox = if defined?(Minitest) || ENV["RACK_ENV"] == "test" || ENV["RAILS_ENV"] == "test" || ENV["CI"] == "true"
                      true
                    else
                      !Aura::CLI::UI.confirm?("❓ Would you like to initialize a new Aura workspace in the current directory?")
                    end
    ...
    def self.initialize_sandbox
      ...
      unless File.directory?(sandbox_aura)
        ...
        _, err, status = Open3.capture3("git", "clone", GlobalConfig.repo_path, sandbox_aura)
        if status.success?
    ...
    def self.initialize_workspace_in_place(target_dir)
      ...
      _, err, status = Open3.capture3("git", "clone", GlobalConfig.repo_path, hidden)
      if status.success?
