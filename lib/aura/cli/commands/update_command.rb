    ...
    class UpdateCommand < Thor
      ...
      def status
        ...
        if local_commit[:stdout].strip == remote_commit[:stdout].strip
          ...
          if diff[:stdout].strip.length.positive?
            puts "\nPending commits:"
            puts diff[:stdout]
          end
        end
      ...
      def project(path_or_name)
        ...
        if projects[path_or_name]
          ...
          projects.each do |name, path|
            next unless File.expand_path(path) == normalized_path
      ...
      def merge
        ...
        has_changes = status_out[:stdout].strip.length.positive?
        ...
        if has_changes
          if options[:force]
            ...
            if res[:success]
              ...
              puts res[:stdout] if res[:stdout].strip.length.positive?
            else
