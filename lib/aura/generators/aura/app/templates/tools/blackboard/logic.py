...
def resolve_bus_dir():
    ...
    if not session_name:
        active_txt = os.path.join(os.getcwd(), "state", "active_session.txt")
    ...
    bus_dir = os.path.join(os.getcwd(), "state", "sessions", session_name, "bus")
        ...
        with open(target_path, "r", encoding="utf-8") as f:
            ...
            try:
                ...
                if isinstance(data, dict) and "data" in data and "metadata" in data:
                    return {"status": "success", "payload": data["data"], "metadata": data["metadata"]}
                return {"status": "success", "payload": data}
            except json.JSONDecodeError:
                return {"status": "success", "payload": content}
    except Exception as e:
    ...
    if action == "lock":
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                return {"status": "success", "message": f"Lock '{key}' acquired."}
        ...
        if os.path.exists(lock_path):
            os.remove(lock_path)
            return {"status": "success", "message": f"Lock '{key}' released."}
        else:
            return {"status": "success", "message": f"Lock '{key}' was not held."}
