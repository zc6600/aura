            ...
            if File.exist?(file)
              ...
              unless content.empty?
                ...
                if content.length > max_file_chars
                  content = content[0,
                                    max_file_chars] + " ... [truncated: exceeds #{max_file_chars} character limit]"
                end
          ...
          begin
            Find.find(@path) do |file|
              if File.directory?(file)
                next if File.expand_path(file) == File.expand_path(@path)

                base = File.basename(file)
                Find.prune if base.start_with?(".") || %w[node_modules vendor tmp log build dist coverage state].include?(base)
