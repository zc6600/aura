    ...
    class Base
      ...
      private
        ...
        def compress_content(content, limit)
          ...
          if out.length > limit
            if @db && @db.respond_to?(:commit_summary)
        ...
        def state_priority_compress(sections, limit)
          ...
          av_block = if av_idx
            current[av_idx, current.length - av_idx]
          end
          ...
          if per_event_cap > 0
            truncated = events.map do |line|
              if line && line.length > per_event_cap
                ...
                max_body = 0 if max_body < 0
                (max_body > 0 ? line[0, max_body] : "") + notice
              else
                line
              end
            end
          ...
          if calc_total(sections) > limit && summary_trim_step > 0
            while calc_total(sections) > limit && events.size > 0
        ...
        def drop_sections_until_fit(sections, limit, order)
          total_len = sections.values.compact.join("\n\n").length
          return sections if total_len <= limit
        ...
        def aggressive_state_trim(sections, limit, order)
          return sections unless sections[:state]
