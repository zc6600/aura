...
def inspect_tool(tool_name):
    ...
    base = os.path.join(".aura-workspace", "tools", safe_name)
    ...
    if not os.path.isdir(base):
        return {"error": f"Tool '{safe_name}' not found in /tools or /.aura-workspace/tools", "status": "failed", "code": "not_found"}
    ...
    if manifest:
        ...
        if "input_schema" in manifest:
            result["input_schema"] = manifest["input_schema"]
    return result
