...
- 打印上下文：
  - `aura context .`
- 检查工具：
  - 结构化：`aura tools inspect <tool>`
  - 美化 JSON：`aura tools inspect <tool> --pretty`
  - 人类可读：`aura tools inspect <tool> --human`
- 运行一次内核：
  - JSON 输出：
...
- 运行规划阶段：
  - `aura kernel plan . -H -n 8`（默认会把上下文与目标传给 LLM，输出下一步工具调用）
- 持续运行直到 final：
  - `aura kernel loop . -g "你的目标" -m 10`（最大步数可选，达到后停止）
...
- `上下文过长`：调整 `config/config.yml` 的 `state_management.max_state_chars` 或触发代谢
...
## 状态文件
- SQLite 数据库：`.aura-workspace/state/sessions/default.db`（支持通过 `aura session` 切换会话存储）
- 查看最近事件：`sqlite3 .aura-workspace/state/sessions/default.db "SELECT id, phase, tool, payload FROM events ORDER BY id DESC LIMIT 10;"`
- 查看最近摘要：`sqlite3 .aura-workspace/state/sessions/default.db "SELECT id, content FROM summaries ORDER BY id DESC LIMIT 5;"`
