 ...
 */
export async function resolveProjectPath(projectPath: string | null): Promise<string> {
 ...
 */
export function safeLoadYaml(filePath: string): any {
...
export async function handleNoWorkspace(startDir: string): Promise<string> {
  console.warn('\x1b[33m⚠️ Warning: Not in an Aura workspace (no .aura folder found in parent directories).\x1b[0m');

  const isTest = process.env.NODE_ENV === 'test' || process.env.CI === 'true';
  let useSandbox = true;

  if (!isTest) {
    const answer = await UI.confirm('❓ Would you like to initialize a new Aura workspace in the current directory?');
    useSandbox = !answer;
  }
...
export async function initializeSandbox(): Promise<string> {
  ...
  console.log(`\x1b[34mℹ️ Routing to global sandbox workspace: ${sandboxPath}\x1b[0m`);
  ...
  if (!fs.existsSync(sandboxAura)) {
    ...
    try {
      ...
      await GlobalConfig.gitRun(sandboxAura, 'config', 'user.name', 'Aura Sandbox');
      await GlobalConfig.gitRun(sandboxAura, 'config', 'user.email', 'sandbox@aura-os.ai');
      ...
      const innerRules = fs.existsSync(innerIgnorePath) ? fs.readFileSync(innerIgnorePath, 'utf-8') : '';
      if (!innerRules.includes('state/aura.db*')) {
        fs.writeFileSync(innerIgnorePath, `${innerRules}\nstate/aura.db*\n`, 'utf-8');
      }
      ...
      console.log('\x1b[32mGlobal sandbox workspace initialized successfully!\x1b[0m');
    } catch (err: any) {
      console.error(`\x1b[31m⛔️ Error: Failed to clone global templates into sandbox workspace:\n${err.message}\x1b[0m`);
      process.exit(1);
    }
  }
...
export async function initializeWorkspaceInPlace(targetDir: string): Promise<string> {
  ...
  try {
    ...
    await GlobalConfig.gitRun(hidden, 'config', 'user.email', 'workspace@aura-os.ai');
    ...
    const existingRules = fs.existsSync(gitIgnorePath) ? fs.readFileSync(gitIgnorePath, 'utf-8') : '';
    if (!existingRules.includes('.aura/')) {
      fs.writeFileSync(gitIgnorePath, `${existingRules}\n.aura/\n`, 'utf-8');
      console.log('\x1b[32mInjected .gitignore rule for hidden .aura environment.\x1b[0m');
    }
    ...
    console.error(`\x1b[31m⛔️ Error: Failed to initialize workspace:\n${err.message}\x1b[0m`);
    process.exit(1);
  }
}
...
export async function initializeGlobalEnv(): Promise<string> {
  ...
  if (!fs.existsSync(globalEnv)) {
    ...
    try {
      ...
      await GlobalConfig.gitRun(globalEnv, 'config', 'user.name', 'Aura Global');
      await GlobalConfig.gitRun(globalEnv, 'config', 'user.email', 'global@aura-os.ai');
      ...
      console.log('\x1b[32mGlobal environment initialized successfully!\x1b[0m');
    } catch (err: any) {
      console.error(`\x1b[31m⛔️ Error: Failed to initialize global environment:\n${err.message}\x1b[0m`);
      process.exit(1);
    }
  }
