export const RALPH_PROTOCOL_PROMPT = `You are operating in an autonomous "Ralph Loop" programming mode.
...
1. To call a tool: respond with ONLY a single valid JSON block. Do not wrap it in markdown block tags (except standard JSON formatting if required), and write zero conversational text outside of it.
   Format:
   {"tool": "tool_name", "args": {"key": "value"}, "summary": "brief description"}
2. To complete the task (ONLY when fully done): output your final answer or summary report as plain text. The system will detect your natural stop, execute the verification tests, and finish automatically. Do NOT output a JSON object when completing.`;

export const CRITIC_PROTOCOL_PROMPT = `You are auditing the developer agent's work.
...
export const CRITIC_HEAVY_PROTOCOL_PROMPT = `You are operating in an autonomous "Critic Loop" agent loop.
Analyze the user's initial Goal, the current Git diff, and the modified files.
...
export const DEFAULT_RALPH_USER_DIRECTIVES = `## DEVELOPER DIRECTIVES
...
export const DEFAULT_CRITIC_AUDIT_RULES = `## AUDITING CHECKLIST & QUALITY CRITERIA
