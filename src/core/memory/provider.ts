...
export class MemoryProvider {
  ...
  public recentEvents(options: { limit?: number | null; phases?: string[] | null; tools?: string[] | null } = {}): any[] {
    return this.store.fetchEvents(options);
  }

  public oldEvents(keepRecent = 20): any[] {
  ...
  public recentSummaries(limit?: number | null): any[] {
    return this.store.fetchSummaries({ limit });
  }

  public activeVariables(): Record<string, any> {
    return this.store.allVariables();
  }

  public assembleContext(include: Array<'events' | 'summaries' | 'variables'> = ['events', 'summaries', 'variables'], options: any = {}): any {
    const context: any = {};
    if (include.includes('events')) {
      context.events = this.recentEvents({ limit: options.event_limit, phases: options.phases });
    }
  ...
  public toMarkdown(options: { summary_limit?: number | null; event_limit?: number | null; event_time_gap_seconds?: number } = {}): string {
    ...
    for (const s of summaries) {
      ...
      historyEntries.push({ ts, seq, order: 2, id: Number(s.id), body: `Summary: ${body}` });
    }
    ...
    // 3. Process Events
    const items = this.recentEvents({ limit: options.event_limit, phases: ['user', 'plan', 'execution'] });
  ...
  private formatVariables(vars: Record<string, any>): string[] {
    const toolStatus: Record<string, any> = {};
    const toolErrors: Record<string, any> = {};
    const otherVars: Record<string, any> = {};
  ...
  private formatEvent(e: any): HistoryEntry | null {
    ...
    switch (phase) {
      case 'user': {
        const txt = (typeof pl === 'object' ? (pl.content ?? pl.text ?? '') : String(pl)).replace(/\s+/g, ' ').trim();
        const seq = (typeof pl === 'object' && pl.call_seq !== undefined && pl.call_seq !== null) ? Number(pl.call_seq) : Number(e.id);
        return { ts, seq, order: 0, id: Number(e.id), body: `User: ${txt}` };
      }
      case 'plan': {
        const planData = typeof pl === 'object' ? pl : {};
        const planTool = planData.tool;
        const summary = planData.summary;
        const thought = planData.thought ?? planData.content ?? (typeof pl === 'string' ? pl : null);

        let body = '';
        if (String(planTool) === 'final') {
          const finalContent = planData.args?.content;
          let txt = String(finalContent ?? '').replace(/\s+/g, ' ').trim();
          if (txt.length > 200) {
      ...
      case 'execution': {
        const res = typeof pl === 'object' ? pl.result : null;
        let status = '';
        if (typeof pl === 'object') {
          const resStatus = res && typeof res === 'object' ? res.status : null;
          const resSuccess = res && typeof res === 'object' ? res.success : null;
          const topStatus = pl.status;
          const topSuccess = pl.success;

          status = resStatus ?? topStatus ?? '';
          if (!status) {
            const success = resSuccess !== undefined && resSuccess !== null ? resSuccess : topSuccess;
            if (success === true) {
              status = 'ok';
        ...
        if (typeof pl === 'object') {
          ...
          if (res && typeof res === 'object') {
            candidates.push(res.output, res.content, res.stdout, res.stderr, res.message);
          }
          ...
          body = found !== undefined ? String(found) : (res ? JSON.stringify(res) : JSON.stringify(pl));
        } else {
          body = String(pl);
        }
        ...
        const seq = (typeof pl === 'object' && pl.call_seq !== undefined && pl.call_seq !== null) ? Number(pl.call_seq) : Number(e.id);
        return { ts, seq, order: 1, id: Number(e.id), body: `Tool ${tool}: ${status} - ${body}` };
      }
      default:
        return null;
    }
  ...
  private formatHistoryEntries(ordered: HistoryEntry[], threshold: number): string[] {
    ...
    const lines = ordered.map(e => {
      ...
      if (ts > 0) {
        ...
        const showTime = showDate || lastTs === null || (Math.abs(ts - lastTs) >= threshold);
