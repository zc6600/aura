...
- Many tests generate a temporary Agent project via `bin/aura new <tmp_path>` and assert files under that generated project (e.g., `config/config.yml`, `tools/read_file/...`).
- When a doc example references `tools/` or `config/config.yml`, it is referring to the generated Agent project root, not necessarily this repository root.
...
- Documentation exists under `docs/` and is kept consistent with implementation.
