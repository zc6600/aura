...
import { EventBus } from '../../src/core/memory/eventBus.js';

class MockRunner {
  public plans: any[] = [];
  public toolResults: any[] = [];
  ...
  public async planStream(goal: string, ctx: string, onEvent?: (ev: any) => void): Promise<any> {
    ...
    const plan = this.plans[this.planIndex] || this.plans[this.plans.length - 1];
    this.planIndex++;
    if (onEvent) {
  ...
  public async runCall(call: any): Promise<any> {
    ...
    const result = this.toolResults[this.toolIndex] || this.toolResults[this.toolResults.length - 1];
    this.toolIndex++;
    return result || { status: 'success', output: 'ok' };
  }

  public async observe(): Promise<any> {
    this.observeCalls.push(true);
    const obs = this.observations[this.obsIndex] || this.observations[this.observations.length - 1];
    this.obsIndex++;
  ...
  public recordUserInput(input: string): void {}
}
...
describe('AgentLoop', () => {
  ...
  beforeEach(() => {
    ...
    loop = new AgentLoop(runner, { eventBus });
  });
  ...
  it('test_aborts_on_max_steps', async () => {
    ...
    runner.plans = Array(10).fill({ tool: 'bash', args: {}, finish_reason: 'tool_calls' });
    runner.toolResults = Array(10).fill({ status: 'success', output: 'ok' });
  ...
  it('test_aborts_on_format_errors', async () => {
    ...
    runner.plans = [
      { thought: 'thinking...', finish_reason: 'tool_calls' },
      { thought: 'still thinking', finish_reason: 'tool_calls' },
    ];
  ...
  it('test_aborts_on_tool_errors', async () => {
    ...
    runner.plans = Array(5).fill({ tool: 'bash', args: {}, finish_reason: 'tool_calls' });
    runner.toolResults = Array(5).fill({ status: 'failed', advice: 'permission denied' });
  ...
  it('test_emits_thought_events', async () => {
    ...
    runner.toolResults = [{ status: 'success', output: 'content' }];
  ...
  it('test_format_error_counter_resets', async () => {
    ...
    runner.toolResults = [{ status: 'success' }];
  ...
  it('test_uses_default_max_steps', async () => {
    runner.config = {};
    runner.plans = Array(35).fill({ tool: 'bash', args: {}, finish_reason: 'tool_calls' });
    runner.toolResults = Array(35).fill({ status: 'success' });
  ...
  it('test_custom_max_steps_parameter', async () => {
    ...
    runner.plans = Array(10).fill({ tool: 'bash', args: {}, finish_reason: 'tool_calls' });
    runner.toolResults = Array(10).fill({ status: 'success' });
  ...
  it('test_handles_symbol_and_string_keys', async () => {
    ...
    runner.toolResults = [{ status: 'success', output: '/home/user' }];
