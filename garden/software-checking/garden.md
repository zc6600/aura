...
1. Locate source code directories (e.g., `lib/`, `src/`) and testing suites (e.g., `test/`, `spec/`).
2. **Run Baseline Tests**: Execute the primary test command (e.g. `bundle exec rspec`, `npm test`) to ensure the codebase is not already broken. If baseline tests fail, log the errors immediately as pre-existing issues.
...
1. Create step anchors under `anchors/` (e.g. `01_check_core.json`) to track progress. Each anchor file must follow this structure:
   ```json
   { "id": "01_check_core", "call_when": ["lib/core/ has been scanned and trivial issues fixed"] }
   ```
2. Spawn checking subagents using the `subagent` tool. Use `async_mode: true` to run multiple checks in parallel:
   - Example:
     ```json
     {
       ...
       "persona": "auditor",
       "goal": "Scan lib/core/ for code quality. Directly fix simple style/lint issues. Log complex structural errors or missing test templates to blackboard key: report_core",
       "async_mode": true,
       "max_steps": 40
     }
     ```
...
2. Spawn coder subagents targeting the complex flaws in `compliance_report.md`:
   - **Continuous Local Testing**: Patcher agents must run RSpec/tests *within their own subagent loops* to verify their changes before reporting success.
   - Example:
     ```json
     {
       "subagent_id": "fix_core_complex",
       "persona": "coder",
       "goal": "Read blackboard key report_core, fix the structural errors in lib/core/, and verify the fixes by running the corresponding tests locally. Do not report success if local tests fail.",
       "max_steps": 50
     }
     ```

### Phase 5: Run Global Tests & Debug Regressions
1. Run the primary global test command (e.g., `bundle exec rspec`).
2. If tests fail:
     ...
     ```json
      {
        ...
        "persona": "debugger",
        "goal": "Analyze the failure of the failing test file (e.g. test_a.rb) and patch the regression. Ensure the test passes locally before returning.",
        "max_steps": 35
      }
     ```
